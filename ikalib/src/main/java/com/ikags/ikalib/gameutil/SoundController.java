...
public class SoundController {
	 ...
	 *Notes:none.
	 ***************************************************************/
	public void initSounds(Context con){
		context=con;
...
//获得声音设备和设备音量
	 ...
	 *Notes:none.
	 ***************************************************************/
	public void loadSfx(String raw,int ID){
//把资源中的音效加载到指定的ID(播放的时候就对应到这个ID播放就行了)
		AssetFileDescriptor descriptor=null;
		try {
			descriptor = context.getAssets().openFd(raw);
			int id=soundPool.load(descriptor, ID);
			//int id=soundPool.load(context, R.raw.bitsfx, ID);
			soundPoolMap.put(ID,id);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
